...
namespace Core
{
    ...
    public:
        ...
        void            ExpandRegion( const cVector3<T>& location )
        {
            ...
            if( mUpperBound.X() < location.X() )
            {
                mUpperBound.X() = location.X();
            }

            if( mUpperBound.Y() < location.Y() )
            {
                mUpperBound.Y() = location.Y();
            }

            if( mUpperBound.Z() < location.Z() )
            {
        ...
        T               SmallestAxialDistance() const
        {
            ...
            T smallest = std::max( distances.X(), distances.Y() );
            smallest = std::max( smallest, distances.Z() );

            return smallest;
        }

        /// <summary>Returns the area</summary>
        T               Area() const
        {
            cVector3<T> distances = mUpperBound - mLowerBound;
        ...
        {
            if( mLowerBound.X() <= location.X() && mLowerBound.Y() <= location.Y() && mLowerBound.Z() <= location.Z() 
    ...
    public:
        ...
        cSphericalRegion3( const cVector3<T>& center ) : mCenter( center ), mRadius( 0 ) { }

        /// <summary>A spherical 3D region centered around { center } and including up to distance radius.</summary>
        cSphericalRegion3( const cVector3<T>& center, T radius ) : mCenter( center ), mRadius( radius ) { }
        ...
        /// <summary>Set the spherical region.</summary>
        void            SetRegion( const cVector3<T>& center, T radius ) { mCenter = center; mRadius = radius; }
    ...
    typedef cEllipsoidRegion3<int> cEllipsoidRegion3i;
    typedef cEllipsoidRegion3<float> cEllipsoidRegion3f;
    typedef cEllipsoidRegion3<double> cEllipsoidRegion3d;
    ...
    public:
        ...
        /// <summary>An elliptical 3D region centered around { center } and including up to distance radius.</summary>
        cCylindricalRegion3( const cVector3<T>& center, const cVector3<T>& axisLengths ) : mCenter( center ), mAxisLength( axisLengths ) { }

        /// <summary>Set the region.</summary>
        void            SetRegion( cVector3<T> center, T radius, T height ) { mCenter = center; mAxisLength = axisLengths; }
    ...
    typedef cCylindricalRegion3<int> cCylindricalRegion3i;
    typedef cCylindricalRegion3<float> cCylindricalRegion3f;
    typedef cCylindricalRegion3<double> cCylindricalRegion3d;
}
