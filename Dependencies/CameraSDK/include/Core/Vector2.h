...
namespace Core
{
    ...
    public:
        ...
        inline void Rotate( T angle )
        {
            ...
            mVals[ 0 ] = x * cos( angle ) - y * sin( angle );
            mVals[ 1 ] = x * sin( angle ) + y * cos( angle );
        }
        ...
        inline Core::cVector2<float> ToFloat() const
        {
            return Core::cVector2f( (float) X(), (float) Y() );
        }

        inline Core::cVector2<double> ToDouble() const
        {
            return Core::cVector2d( (double) X(), (double) Y() );
        }
    ...
    typedef cVector2<float> CORE_API cVector2f;
    typedef cVector2<double> CORE_API cVector2d;
    ...
    template< typename T>
    std::wostream& operator<<( std::wostream& os, const cVector2<T>& vec )
    {
        os << L"(" << vec[0] << L"," << vec[1] << L")";
        return os;
    }

    template<typename T>
    std::ostream& operator<<( std::ostream& os, const cVector2<T>& v )
    {
        T x = v.X();
        T y = v.Y();
    ...
    template<typename T>
    std::istream& operator >> ( std::istream& is, cVector2<T>& v )
    {
        T x, y;
