...
namespace CameraLibrary
{
    ...
    public:
        ...
        int             IMUTelemetryCount();
        cIMUTelemetry * IMUTelemetry( int index );

        //== Hardware Based Timing Information ====--

        double          HardwareTimeStampValue(); //== Hardware Time Stamp ( in seconds ) ====---
        ...
        //== Direct Compressed Image Access =============================--
        //== 
        //== The CompressedImage function will return a buffer that is a compressed image
        //== if the camera is in a scene video mode that utilizes compression.  When the
        //== camera is in MJPEG Mode, this buffer is a full JPEG image.  When the camera
        //== is in Video Mode, this buffer is a full frame of H.264 encoded imagery.
        //==
        //== Requirement: You need to set late MJPEG decompression for the camera(s) 
        //== via: camera->SetLateDecompression(true);

        int             CompressedImageSize();
        int             CompressedImage(unsigned char *Buffer, int BufferSize);
