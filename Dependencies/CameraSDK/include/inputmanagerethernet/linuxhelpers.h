		...
		while (arp.good())
		{
					...
					adapterName.compare(device) == 0)
			{
				...
				if (address.find(mac) != std::string::npos)
				{
					...
					// Succeed
					return (atoi( iptokens[ 2 ].c_str() )<<8)|atoi(iptokens[3].c_str());
				}
			}
		}
	}
...
inline bool ArpVerifyClean(int byte1, int lsb, const std::string &adapterName = "")
{
	// The arp table
	std::ifstream arp("/proc/net/arp");
	...
	try
	{
		...
		while (arp.good())
		{
					...
					adapterName.compare(device) == 0)
			{
				...
				// Found the LSB
				if (iptokens.size() >= 4 && atoi(iptokens[3].c_str()) == lsb && atoi( iptokens[ 2 ].c_str() ) == byte1 )
					return false;
			}
		}
	}
